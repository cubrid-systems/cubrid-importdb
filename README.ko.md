...
스키마의 의존 그래프에서 적재 순서를 직접 계산하고, 제약 조건이 없는 맨 힙에 데이터를
...
전체 데이터베이스 재적재의 조작 인터페이스는 이게 전부입니다. `unloaddb`가 써 놓은
...
곤란한 데이터에 들이대기 전에 사본으로 먼저 평가해 보십시오.
...
**시작하기 전에**, 이 도구를 여러분 환경에서 쓸 수 있는지를 가르는 네 가지입니다.
...
| **덤프 디렉터리** | **쓰기 가능**해야 합니다. 재개용 매니페스트와, 위반이 있을 경우 예외 산출물이 덤프 옆에 기록됩니다. 읽기 전용으로 마운트한 NFS 덤프는 그대로는 쓸 수 없습니다. |
...
**쓰기** — [요구 사항](#요구-사항) · [설치](#설치) · [첫 임포트](#첫-임포트) ·
...
작은 데이터베이스 하나에 명령 세 번, 그리고 그 순서를 틀리지 않는 것도 여러분 책임입니다.
여기서 두 가지를 잃습니다.
...
CUBRID ImportDB는 이 자리를 명령 하나로 대체합니다. 의존 그래프에서 적재를 계획하고, 맨
힙에 넣은 뒤 제약 조건을 일괄로 만들며, 데이터가 만족시키지 못하는 외래 키는 정의하기를
거부합니다 — 어느 행이 문제인지를 정확히 알려주면서.

![제약 조건의 생애, 두 가지 방식](assets/lifecycle.svg)

*그림 1 — 같은 12개 행, 두 가지 방식. 제약 조건을 살려 두면 행마다, 인덱스마다 b-tree
쓰기가 한 번씩 들고도 외래 키는 검사되지 않은 채로 남습니다. 제약을 떼어내고 나중에
만들면 그 만드는 과정 자체가 검사가 됩니다 — 한쪽 레인에서는 고아 행이 잡히고 다른 쪽에서는
조용히 남는 이유가 이것입니다.*
...
**힙에만 넣는 적재.** PK/UNIQUE/FK를 떼어내고 맨 힙에 넣은 다음, 행이 다 찬 테이블 위에서
...
**HA 문제를 숨기지 않습니다.** `ha_mode=on`인 대상은 기본적으로 거부합니다. 맨 힙 적재는
스탠바이로 복제되지 않기 때문입니다. `--allow-ha`로 넘길 수 있고, 그 경우 적재 전에 한 번,
판정에서 한 번, 두 번 경고합니다.

**`--dry-run`.** 계획과 말단 작업 순서를 출력하고 아무것도 바꾸지 않습니다.

## 동작 방식

**Discover → Define → Graph → Plan → Strip → Load → Rebuild → FK define → Stats →
Triggers.** 중요한 모양은 가운데에 있습니다. 스키마를 정의하고, 그 제약 조건 집합을
스냅숏으로 떠 둔 다음 *떼어내고*, 데이터는 맨 힙으로 들어가고, 제약 조건은 나중에 일괄로
만들어집니다. 트리거는 엄격하게 맨 마지막에 정의되므로 적재 도중에는 아무것도 발화하지
않습니다.
...
열일곱 개의 말단 작업은 실제 실행이 낸 것으로 [docs/output.md](docs/output.md)에
...
흘러가는데, 그 블록은 출력된 줄들이 답하지 못하는 것 — 지금 어느 단계이고, 몇 개가 남았고,
그 안에서 얼마나 진행되었는지 — 를 답해 줍니다.
...
표시는 꺼지므로 파이프나 파일, CI 로그에는 예전 그대로의 평범한 출력만 갑니다.
...
만들어지는 보조 인덱스보다 시점이 앞섭니다. 그것이 질의 계획에 얼마를 물리는지는 여기서
...
`tests/run_tests.sh crossversion`이 그 증거이며, 실제 10.2 설치본 위에서 돌아갑니다. 데이터는
10.2 원본과, 카탈로그와 권한 집합은 같은 픽스처의 *현행* 덤프로 만든 대상과 비교되며 전부
바이트 단위로 동일합니다.

하한선의 근거, 가드가 걸린 형태들, 그리고 가드를 걸 수 없는 두 가지는
...
생애 차이만 떼어냅니다. `tests/run_tests.sh perf` **자체가** 그 측정이며, 별도의 벤치마크는
없습니다.
...
- **`--degree`에는 `--datafile-per-class` 덤프가 필요합니다.** 펼쳐지는 단위가 오브젝트
  파일이므로, 기본 단일 파일 덤프는 무엇을 넘기든 직렬로 돕니다. 이 경우 타이밍을 보고
  짐작하게 두는 대신 실행이 그렇다고 말해 줍니다.
- **이보다 열 배 큰 규모에서는 양상이 달라집니다.** 외래 키 정의가 실행의 절반을 차지하고
  `--degree`는 더 이상 도움이 되지 않는데, 원인은 importdb가 하는 무엇이 아니라
...
가장 빠른 길은 nightly 드롭입니다. 같은 빌드에서 나온 소스와 설치본을 한 쌍으로 배포합니다.
...
`$CUBRID/cci/lib`를 바이너리의 `RUNPATH`에 구워 넣는데, `libcubridcs`가 `libcascci`에
...
# /dumps/mydb는 쓰기 가능해야 합니다: 매니페스트와, 위반이 있으면 예외 산출물이
# 그 자리에 기록됩니다.
...
카탈로그에서 읽히기 때문입니다. 이미 있던 클래스는 임포트의 일부로 취급되어 — 데이터 단계
전에 PK/UNIQUE/FK가 떨어지고 이후에 재구축되며, 그 사이에 실행이 중단되면 떨어진 채로 남습니다.
...
전에 연결이 실패합니다. 비밀번호를 넘기면 그것은 명령행에 실립니다 — importdb 자신의 명령행과,
importdb가 fork 하는 `cub_admin loaddb` 자식들의 명령행에 — 그래서 그 호스트에서 `/proc`를
...
**하나의 덤프를 두 대상에 넣으려면 `--restart`가 필요하고, 빼먹으면 조용히 실패합니다.**
...
대상을 위해 쓰인 매니페스트를 설명합니다. 측정해 보면 아예 존재하지도 않는 데이터베이스를 두고
...
고쳐지기 전까지는 `--restart`를 넘기거나(매니페스트를 보기 전에 검사되므로 두 번째 대상을
깨끗하게 시작하고 이전 매니페스트는 옆에 보존합니다), 실행 사이에 `importdb.manifest`를
지우거나, 대상마다 덤프 디렉터리 사본을 따로 주십시오. 하나의 덤프를 여러 데이터베이스에 넣는
...
- **`unloaddb`가 덤프에 담지 않은 것은 옮기지 못합니다.** 덤프에 있는 것은 재생되고, 그 범위는
  ...
  복원된 계정으로 로그인해 확인합니다. `unloaddb`가 쓰지 않는 것 — 이를테면 저장 프로시저 —
  은 importdb가 지어낼 수 없습니다.
- **HA 스탠바이로 복제하지 않습니다.** 맨 힙 적재는 행 복제를 만들지 않으므로, `ha_mode=on`
  대상은 `--allow-ha`를 넘기고 이후 백업으로 스탠바이를 재구성하지 않는 한 거부됩니다.
- **잘린 오브젝트 파일과 원래 짧은 덤프를 구별하지 못합니다.** `unloaddb` 덤프에는 클래스별
  행 수가 들어 있지 않으므로, 그 아래의 무엇도 — `loaddb`든 importdb든 — 중간에서 잘린 파일과
  원래 행이 적은 파일을 구별할 수 없습니다. 그래서 잘린 오브젝트 파일은 **exit 0**으로
  임포트되고 `COMPLETE`로 보고됩니다. 측정해 보면 41행짜리 오브젝트 파일을 절반으로 자르자
  20행을 적재하고 끝났다고 했습니다. 그러니 임포트 전에 덤프를 검증하고, 이후에 원본과 행 수를
  대조하십시오. 리포트의 `loaded:` 블록이 오브젝트 파일당 하나씩 수치를 주는데,
  `--datafile-per-class` 덤프에서는 클래스별이고 기본 덤프에서는 전체 합계 하나입니다. 이것을
  닫으려면 행 수가 덤프에 기록되어야 하고, 그것은 importdb가 아니라 `unloaddb`의 변경입니다.
...
# 크로스버전 레인: 덤프를 만들 '더 오래된' 두 번째 설치본이 필요하다
...
- [`tests/`](tests/README.md) — 14개 케이스, 성능 케이스를 뺀 285개 단언(성능 케이스는 측정
  쌍마다 자체 단언을 더합니다): 왕복 정합성, 모든 컬럼 타입 계열, 의존 순서, 외래 키 순환,
...
- `docs/` — [`output.md`](docs/output.md)(이 도구가 출력하는 모든 것),
  ...
  [`old-dumps.md`](docs/old-dumps.md)(10.2 하한선과 가드가 걸린 형태들),

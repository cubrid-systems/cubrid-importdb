...
| `dupname` | The same constraint name on more than one class &mdash; four classes each with a PRIMARY KEY named `pk1`, two of them a UNIQUE named `u1`, and two of them a FOREIGN KEY named `fk1`, all of which CUBRID allows because constraint names are unique per class rather than per database. Every one of them must be back at the end (catalog fingerprint) **and** recorded against the class it belongs to (the manifest's `[rebuild]` section, which is what a repair is driven from). One class is also named `dn add e`, because a delimited class name may carry the ADD keyword the statement matcher looks for. Run against both schema layouts &mdash; the DEFAULT single interleaved schema file and the SPLIT `_schema_pk`/`_schema_uk` pair &mdash; since the same class-and-name key resolves the identities out of both. The FK half is the sharper one: a collapsed edge entry did not merely mis-record, it dropped the second statement as already processed and never defined that FOREIGN KEY. |
...
| `refusals` | The things that must be refusals rather than surprises: no arguments, a missing positional, a missing dump directory, a directory with no dump in it, a dump with a schema file but no object roster, `--exceptions-table` (reserved), a non-DBA user, and a non-empty target &mdash; both a class whose name collides with the dump's and one the dump does not define at all. Each asserts the non-zero exit *and* the specific diagnostic, so a refusal that starts happening for a different reason still fails. Two things the refusal must *not* do are asserted as well: it must not name a partition pseudo-class (they are in `db_class` and cannot be dropped), and it must leave the dump directory as it found it, so re-running the identical command refuses again rather than reading its own leftover manifest as an interrupted import. |

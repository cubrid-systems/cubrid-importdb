...
namespace
{
  ...
  const entry importdb_messages[] = {
    ...
    { 41, "importdb: FK re-validation found %1$d orphan row(s) on '%2$s' -> '%3$s' [%4$s]; offenders written to %5$s. importdb exits non-zero (fail-fast; use --continue to enumerate every violated edge).\n" },
    { 42, "importdb: FK re-validation of '%1$s' passed -- %2$d edge(s) validated clean, %3$d skipped (parent key withheld). No FK is defined yet (a later phase).\n" },
    { 43, "importdb: FK re-validation of '%1$s' found violations -- %2$d edge(s) violated, %3$ld orphan row(s) total; offenders written to %4$s. See the manifest [validate] section; importdb exits non-zero.\n" },
    { 44, "importdb: FK re-validation anti-join failed on '%1$s' -> '%2$s' [%3$s]: %4$s\n" },
    ...
    { 47, "importdb: defined %1$d FK(s), withheld %2$d on '%3$s' (validation orphan or parent key un-rebuilt); re-add DDL recorded in the manifest [fkdefine] section and %4$s. importdb exits non-zero.\n" },
    { 48, "importdb: defining FK [%1$s] on '%2$s' -> '%3$s' failed unexpectedly (the data validated clean): %4$s\n" },
